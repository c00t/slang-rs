...
include_cpp! {
    ...
    extern_cpp_type!("SlangDeclKind",crate::cbind::SlangDeclKind)
    pod!("SlangDeclKind")
    ...
    generate!("slang::GenericReflection")
}

use ffi::{
    slang::{BufferReflection, DeclReflection, FunctionReflection, GenericReflection},
    SlangDeclKind,
};
// reexport
pub use ffi::slang::{
...
unsafe impl cxx::ExternType for crate::cbind::SlangDeclKind {
    type Id = cxx::type_id!("SlangDeclKind");
    type Kind = cxx::kind::Trivial;
}
...
impl ProgramLayout {
    ...
    pub fn find_function_by_name(
        self: Pin<&mut Self>,
        name: &str,
    ) -> Option<Pin<&mut FunctionReflection>> {
        let name = std::ffi::CString::new(name).unwrap();
        let result = unsafe { self.findFunctionByName(name.as_ptr()) };
        if result.is_null() {
            None
        } else {
            Some(unsafe { Pin::new_unchecked(&mut *result) })
    ...
    pub fn find_function_by_name_in_type(
        self: Pin<&mut Self>,
        type_ref: Pin<&mut TypeReflection>,
        name: &str,
    ) -> Option<Pin<&mut FunctionReflection>> {
        let name = std::ffi::CString::new(name).unwrap();
        let result =
            unsafe { self.findFunctionByNameInType(type_ref.get_unchecked_mut(), name.as_ptr()) };
        if result.is_null() {
            None
        } else {
            Some(unsafe { Pin::new_unchecked(&mut *result) })
    ...
    pub fn find_var_by_name_in_type(
        ...
        type_ref: Pin<&mut TypeReflection>,
        name: &str,
    ) -> Option<Pin<&mut VariableReflection>> {
        let name = std::ffi::CString::new(name).unwrap();
        let result =
            unsafe { self.findVarByNameInType(type_ref.get_unchecked_mut(), name.as_ptr()) };
        if result.is_null() {
            None
        } else {
            Some(unsafe { Pin::new_unchecked(&mut *result) })
    ...
    pub fn is_sub_type(
        self: Pin<&mut Self>,
        sub_type: Pin<&mut TypeReflection>,
        super_type: Pin<&mut TypeReflection>,
    ) -> bool {
        unsafe { self.isSubType(sub_type.get_unchecked_mut(), super_type.get_unchecked_mut()) }
    }
...
impl TypeLayoutReflection {
    ...
    pub fn get_field_binding_range_offset(self: Pin<&mut Self>, field_index: i64) -> i64 {
        unsafe { self.getFieldBindingRangeOffset(field_index) }
    }
...
impl EntryPointReflection {
    ...
    pub fn get_function(self: Pin<&mut Self>) -> Pin<&mut FunctionReflection> {
        unsafe { Pin::new_unchecked(&mut *self.getFunction()) }
    }
...
impl VariableLayoutReflection {
    ...
    pub fn find_modifier(self: Pin<&mut Self>, id: Modifier_ID) -> Pin<&mut Modifier> {
        unsafe { Pin::new_unchecked(&mut *self.findModifier(id)) }
    }
...
impl VariableReflection {
    ...
    pub unsafe fn find_user_attribute_by_name(
        self: Pin<&mut Self>,
        global_session: *mut c_void,
        name: &str,
    ) -> Option<Pin<&mut UserAttribute>> {
        unsafe {
            let name = std::ffi::CString::new(name).unwrap();
            let ptr = self.findUserAttributeByName(
                global_session as *mut ffi::slang::IGlobalSession,
                name.as_ptr(),
            );
            // TODO: Add checks for all other unsafe functions which return pointers
            if ptr.is_null() {
                None
            } else {
    ...
    pub fn has_default_value(self: Pin<&mut Self>) -> bool {
        unsafe { self.hasDefaultValue() }
    }

    pub fn get_generic_container(self: Pin<&mut Self>) -> Pin<&mut GenericReflection> {
        unsafe { Pin::new_unchecked(&mut *self.getGenericContainer()) }
    }

    pub fn apply_specializations(
        self: Pin<&mut Self>,
        generic: Pin<&mut GenericReflection>,
    ) -> Pin<&mut VariableReflection> {
        unsafe { Pin::new_unchecked(&mut *self.applySpecializations(generic.get_unchecked_mut())) }
    }
}
...
impl DeclReflection {
    ...
    pub fn as_function(self: Pin<&mut Self>) -> Pin<&mut FunctionReflection> {
        unsafe { Pin::new_unchecked(&mut *self.asFunction()) }
    }

    pub fn as_generic(self: Pin<&mut Self>) -> Pin<&mut GenericReflection> {
        unsafe { Pin::new_unchecked(&mut *self.asGeneric()) }
    }

    pub fn get_parent(self: Pin<&mut Self>) -> Pin<&mut DeclReflection> {
        unsafe { Pin::new_unchecked(&mut *self.getParent()) }
    }
}
...
impl FunctionReflection {
    ...
    pub fn get_return_type(self: Pin<&mut Self>) -> Pin<&mut TypeReflection> {
        unsafe { Pin::new_unchecked(&mut *self.getReturnType()) }
    }
    ...
    pub fn is_overloaded(self: Pin<&mut Self>) -> bool {
        unsafe { self.isOverloaded() }
    }

    pub fn get_overload_count(self: Pin<&mut Self>) -> u32 {
        unsafe { self.getOverloadCount() }.into()
    }

    pub fn get_overload(self: Pin<&mut Self>, index: u32) -> Pin<&mut FunctionReflection> {
        unsafe { Pin::new_unchecked(&mut *self.getOverload(c_uint(index.into()))) }
    }
}
...
impl GenericReflection {
    ...
    pub fn as_decl(self: Pin<&mut Self>) -> Pin<&mut DeclReflection> {
        unsafe { Pin::new_unchecked(&mut *self.asDecl()) }
    }

    pub fn get_type_parameter_count(self: Pin<&mut Self>) -> u32 {
        unsafe { self.getTypeParameterCount() }.into()
    }

    pub fn get_type_parameter(self: Pin<&mut Self>, index: u32) -> Pin<&mut VariableReflection> {
        unsafe { Pin::new_unchecked(&mut *self.getTypeParameter(c_uint(index.into()))) }
    }

    pub fn get_value_parameter_count(self: Pin<&mut Self>) -> u32 {
        unsafe { self.getValueParameterCount() }.into()
    }

    pub fn get_value_parameter(self: Pin<&mut Self>, index: u32) -> Pin<&mut VariableReflection> {
        unsafe { Pin::new_unchecked(&mut *self.getValueParameter(c_uint(index.into()))) }
    }

    pub fn get_type_parameter_constraint_count(
        self: Pin<&mut Self>,
        type_param: Pin<&mut VariableReflection>,
    ) -> u32 {
        unsafe { self.getTypeParameterConstraintCount(type_param.get_unchecked_mut()) }.into()
    }

    pub fn get_type_parameter_constraint_type(
        self: Pin<&mut Self>,
        type_param: Pin<&mut VariableReflection>,
        index: u32,
    ) -> Pin<&mut TypeReflection> {
        unsafe {
            Pin::new_unchecked(&mut *self.getTypeParameterConstraintType(
                type_param.get_unchecked_mut(),
                c_uint(index.into()),
            ))
        }
    }

    pub fn get_inner_decl(self: Pin<&mut Self>) -> Pin<&mut DeclReflection> {
        unsafe { Pin::new_unchecked(&mut *self.getInnerDecl()) }
    }

    pub fn get_inner_kind(self: Pin<&mut Self>) -> SlangDeclKind {
        unsafe { self.getInnerKind() }
    }

    pub fn get_outer_generic_container(self: Pin<&mut Self>) -> Pin<&mut GenericReflection> {
        unsafe { Pin::new_unchecked(&mut *self.getOuterGenericContainer()) }
    }
    ...
    pub fn get_concrete_type(
        ...
        unsafe { Pin::new_unchecked(&mut *self.getConcreteType(type_param.get_unchecked_mut())) }
    }

    pub fn get_concrete_int_val(
        self: Pin<&mut Self>,
        value_param: Pin<&mut VariableReflection>,
    ) -> i64 {
        unsafe { self.getConcreteIntVal(value_param.get_unchecked_mut()) }.into()
    }

    pub fn apply_specializations(
        self: Pin<&mut Self>,
        generic: Pin<&mut GenericReflection>,
    ) -> Pin<&mut GenericReflection> {
        unsafe { Pin::new_unchecked(&mut *self.applySpecializations(generic.get_unchecked_mut())) }
    }
}
